...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, sentence_label=None, aspect_label=None,emotion=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids_spc, input_mask, segment_ids, label_id, valid_ids=None,
...
def readfile(filename):
    ...
    for line in f:
        if len(line) == 0 or line.startswith('-DOCSTART') or line[0] == "\n":
            if len(sentence) > 0:
                data.append((sentence, tag, emotion))  # Modify this line
                sentence = []
                tag = []
                emotion = []
            continue
    ...
    # print(sentence)
    if len(sentence) > 0:
        data.append((sentence, tag, emotion))
    return data
...
class ATEProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        examples = []
        for i, (sentence, tag,emotion) in enumerate(lines):
            aspect = []
            aspect_tag = []
            aspect_emotion = [-1]
            for w, t, e in zip(sentence, tag, emotion):
                if e != -1:
                    aspect.append(w)
                    aspect_tag.append(t)
